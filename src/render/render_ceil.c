...
static void		loop(t_engine *engine, int *y_top, int *y_bot, int *x_table)
{
	...
	while (x <= engine->vpceil.maxx)
	{
		...
		while (*y_top < y1)		//если верхняя линия опускается
			render_hline(engine, *y_top, x_table[(*y_top)++], x, 4);
		while (*y_bot > y2)		//если нижняя линия поднимается
			render_hline(engine, *y_bot, x_table[(*y_bot)--], x, 4);
		x++;
	}
}
...
void			render_ceil(t_engine *engine)
{
	...
	engine->rend_plane.z = engine->player.where.z - engine->sectors[engine->present->sectorno].ceil;
	y = y_top;
	while (y <= y_bot)
	...
	while (y <= y_bot)
	{
		render_hline(engine, y, x_table[y], engine->vpceil.maxx, 4);
		y++;
	}
}