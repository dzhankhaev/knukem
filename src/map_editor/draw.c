...
void	draw_map(SDL_Renderer *rnd, t_sect *sect, t_all *all)
{
	...
	while(i < all->num_sectors)
	{
		...
		{
			s = (t_xyz){(temp->vertex[j].x * all->step) + all->area.w/2 - (round(all->mapsize.x/2) * all->step),
				(temp->vertex[j].y * all->step) + all->area.h/2 - (round(all->mapsize.y/2) * all->step), 0};
					// (temp->floor - all->mapsize.z/4) * all->step/2};
			f = (t_xyz){(temp->vertex[j + 1].x * all->step) + all->area.w/2 - (round(all->mapsize.x/2) * all->step),
				(temp->vertex[j + 1].y * all->step) + all->area.h/2 - (round(all->mapsize.y/2) * all->step), 0};
					// (temp->floor - all->mapsize.z/4) * all->step/2};
			if (all->iso)
			{
			...
			if (temp->floor >= all->draw_floors.x && temp->floor <= all->draw_floors.y)
			{
				...
				draw_circle(rnd, (t_xy){s.x, s.y}, 2, GREEN);
			}
			j++;
		}
...
void	draw_temp(t_all *all, SDL_Renderer *rnd, t_sect *temp)
{
	...
	while (j < temp->npoints)
	{
		if(j == temp->npoints - 1)
		{
			...
			(temp->vertex[j].y * all->step) + all->area.h/2 - (all->mapsize.y/2 * all->step), 0};
				// (temp->floor - all->mapsize.z/4) * all->step/2};
			f = (t_xyz){(x * all->step) + all->area.w/2 - (all->mapsize.x/2 * all->step),
			(y * all->step) + all->area.h/2 - (all->mapsize.y/2 * all->step), 0};
				// (temp->floor - all->mapsize.z/4) * all->step/2};
		}
		else
...
void	draw_area(SDL_Renderer *rnd, t_all *all)
{
	...
	if (!all->player.picked)
		draw_circle(all->sdl->renderer, (t_xy){all->point.x * all->step + c.x, 
			all->point.y * all->step + c.y}, 2, WHITE);
	if (all->temp->npoints != 0)
	 	draw_temp(all, rnd, all->temp);
	draw_player(all, rnd, &all->player, &c);
}

void	draw_buttons(t_all *all, SDL_Renderer *rnd, t_button *btn)
{
	int i;
...
void	draw_all(t_all *all, SDL_Renderer *rnd, t_button *btn)
{
	...
	draw_digits(rnd, all, all->draw_floors.x, all->draw_floors.y);
	draw_buttons(all, rnd, btn);
	draw_area(rnd, all);
    SDL_RenderPresent(all->sdl->renderer);
}
